(ns ocpp_charging.main
  "Kotodama WASM entrypoint for the Ocpp_charging clean-room actor (L4) — Clojure port.
  ...
  facts (`ocpp_charging.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ocpp_charging")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ocpp_charging.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ocpp_charging-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ocpp_charging-compat/ready)
